 ...
 *
 * History:
 *    06-Apr-1999 - Created by Vitaly Kruglikov (vkruglikov@handspring.com)
...
FlashEntryFuncType			FlashAMDEntryPoint;
...
start (UInt32 selId, void* argP, UInt32 argSize)
{
	return FlashAMDEntryPoint (selId, argP, argSize);
}
...
// API
static FlashOpenFuncType			PrvFlashAMDOpen;

static FlashCloseFuncType			PrvFlashAMDClose;

static FlashAttrGetFuncType			PrvFlashAMDAttrGet;

static FlashExistsFuncType			PrvFlashAMDExists;

static FlashSectorEraseFuncType		PrvFlashAMDSectorErase;

static FlashChipWriteFuncType		PrvFlashAMDChipWrite;

static FlashSameDeviceFuncType		PrvFlashAMDSameDevice;
...
#if (ERROR_CHECK_LEVEL == ERROR_CHECK_FULL)
  static void
  PrvErrorCheckGlobals (FlashAMDGlobalsType* gP);
#endif


static DWord
PrvGetFlashDeviceID (FlashAMDGlobalsType* gP, void* chipBaseP, Byte* manufP,
					Byte* deviceP);

// <chg 25-apr-00 dia> Only do this in debug builds...
#if ERROR_CHECK_LEVEL == ERROR_CHECK_FULL
  static DWord
  PrvGetSectorInfoFromOffset (FlashAMDGlobalsType* gP, UInt32 offsetFromChipBase,
							  FlashSectorInfoType* sectorInfoP, UInt32* sectorIndexP);
#endif // ERROR_CHECK_LEVEL == ERROR_CHECK_FULL

static const FlashAMDDeviceTableEntryType*
PrvFlashAMDIdentify (FlashAMDGlobalsType* gP, void* chipBaseP, DWord* errP);

static DWord
PrvFlashAMDProgramAPILoad (FlashAMDGlobalsType* gP);

static void
PrvFlashAMDProgramAPIUnload (FlashAMDGlobalsType* gP);


static DWord
PrvFlashAMDAttrGet (void* refP, void* chipBaseP, UInt32 attrId,
 ...
 ****************************************************************/
DWord
FlashAMDEntryPoint (UInt32 selId, void* argP, UInt32 argSize)
{
  DWord					err = 0;
  ...
  switch (selId)
	  ...
	  case flashEntrySelGetInterface:
		  ...
		  apiP->FlashEntry		= &FlashAMDEntryPoint;
		  apiP->FlashOpen			= &PrvFlashAMDOpen;
		  apiP->FlashClose		= &PrvFlashAMDClose;
		  apiP->FlashAttrGet		= &PrvFlashAMDAttrGet;
		  apiP->FlashExists		= &PrvFlashAMDExists;
		  apiP->FlashSectorErase	= &PrvFlashAMDSectorErase;
		  apiP->FlashChipWrite	= &PrvFlashAMDChipWrite;
		  apiP->FlashSameDevice	= &PrvFlashAMDSameDevice;
		  }
		  break;
 ...
 *
 *	History:
 *    08-Apr-1999	VMK		Created
 *	  14-Apr-2000	vmk		Initialize the device table logic
 ****************************************************************/
static DWord
PrvFlashAMDOpen (void** refPP, UInt16 codeDbCardNo, LocalID codeDbId,
				 UInt16 codeResId, void* chipBaseP)
{
  DWord		err = 0;
  FlashAMDGlobalsType*	gP = NULL;
  const FlashAMDDeviceTableEntryType*	  descP = NULL;
  ...
  *refPP = NULL;
  ...
  gP = (FlashAMDGlobalsType*)MemPtrNew (sizeof(FlashAMDGlobalsType));
  if (gP == NULL)
	  {
	  err = flashErrNoMemory;
	  goto Exit;
	  }
  MemSet (gP, sizeof(FlashAMDGlobalsType), 0);

  // Initialize signatures for debugging
  gP->startSignature = flashAMDGlobalsStartSignature;
  gP->endSignature = flashAMDGlobalsEndSignature;
  ...
  err = PrvFlashAMDProgramAPILoad (gP);
  if (err)
	  goto Exit;


  // Identify the flash device and check if we support it
  gP->device.deviceEntryP = PrvFlashAMDIdentify (gP, chipBaseP, &err);
  if (err)
	  goto Exit;
 ...
 ****************************************************************/
static DWord
PrvFlashAMDClose (void* refP)
{
  DWord		err = 0;
  FlashAMDGlobalsType*	gP = (FlashAMDGlobalsType*)refP;
  ...
  PrvFlashAMDProgramAPIUnload (gP);

  // Finally, free our private globals chunk
  
  // But first, reset the signatures (for debugging)
  gP->startSignature = flashAMDGlobalsDiscardSignature;
  gP->endSignature = flashAMDGlobalsDiscardSignature;
 ...
 *	  14-Apr-2000	vmk		Use device table-based logic
 ****************************************************************/
static DWord
PrvFlashAMDAttrGet (void* refP, void* chipBaseP, UInt32 attrId,
			  void* argP, UInt32 argSize)
{
  ...
  switch (attrId)
	  ...
	  case flashAttrSectorIndex:
		{
		  const FlashAMDDeviceTableEntryType*  entryP = gP->device.deviceEntryP;
		  ...
		  err = FlashAMDGetSectorIndex (entryP, dataP);
		  ErrNonFatalDisplayIf (err, "FlashAMDGetSectorIndex failed");
		}
		break;
	  ...
	  case flashAttrSectorInfo:
		{
		  ...
		  err = FlashAMDGetSectorInfo (entryP, dataP);
		  ErrNonFatalDisplayIf (err, "FlashAMDGetSectorInfo failed");
		}
		break;
	  ...
	  case flashAttrSectorLock:
		{
		  ...
		  err = FlashAMDGetSectorInfo (entryP, &info);
		  ErrNonFatalDisplayIf (err, "FlashAMDGetSectorInfo failed");
		  if (err)
			  break;
	  ...
	  case flashAttrFlashPartNo:
		{
		  ...
		  const FlashAMDDeviceTableEntryType*	entryP;
		  const char*	  nameP = "";
		  ...
		  entryP = FlashAMDDeviceTableEntryFind (manufId, deviceId, &err);
		  if (entryP)
			{
			  // Keep FlashAMDDeviceTableEntryFind honest
 ...
 ****************************************************************/
static DWord
PrvFlashAMDExists (void* refP, void* chipBaseP)
{
  DWord		err = 0;
  FlashAMDGlobalsType*	gP = (FlashAMDGlobalsType*)refP;
  ...
  PrvFlashAMDIdentify (gP, chipBaseP, &err);
...
/***************************************************************
 *	Function:	PrvFlashAMDSectorErase
 *
 *	Summary:      
 ...
 *	History:
 *    08-Apr-1999	VMK		Created
 *	  18-Apr-2000	vmk		Replace bottom boot block-specific
 *							 error-checking code with dynamically-
 *							 calculated sector info code.
 ****************************************************************/
static DWord
PrvFlashAMDSectorErase (void* refP, void* chipBaseP, void* sectorBaseP)
{
  DWord		err = 0;
  FlashAMDGlobalsType*	gP = (FlashAMDGlobalsType*)refP;
...

  {
	...
	while (wordsLeft--)
		{
		ErrNonFatalDisplayIf (*dstWordP++ != 0xFFFF, "erase failed");
		}
  }
...
} // PrvFlashAMDSectorErase



/***************************************************************
 *	Function:	PrvFlashAMDChipWrite
 *
 *	Summary:      
 ...
 ****************************************************************/
static DWord
PrvFlashAMDChipWrite (void* refP, void* chipBaseP, void* dstP,
 ...
 ****************************************************************/
static Byte
PrvFlashAMDSameDevice (void* refP, void* chip0BaseP, void* chip1BaseP)
{
  Byte		same;
  FlashAMDGlobalsType*	gP = (FlashAMDGlobalsType*)refP;
...
} // PrvFlashAMDSameDevice



/***************************************************************
 *	Function:	PrvFlashAMDIdentify
 *
 *	Summary:
...
PrvFlashAMDIdentify (FlashAMDGlobalsType* gP, void* chipBaseP, DWord* errP)
{
  ...
  *errP = PrvGetFlashDeviceID (gP, chipBaseP, &manufId, &deviceId);
  ...
  entryP = FlashAMDDeviceTableEntryFind (manufId, deviceId, errP);
  if (*errP)
	  goto Exit;
							...
							FlashSectorInfoType* sectorInfoP, UInt32* sectorIndexP)
{
  ...
  err = FlashAMDGetSectorIndex (gP->device.deviceEntryP, &indexInfo);
  if (err)
	{
  ...
  err = FlashAMDGetSectorInfo (gP->device.deviceEntryP, sectorInfoP);
  if (err)
	{
 ...
 *
 *	History:
 *    10-Apr-1999	VMK		Created
 ****************************************************************/
static DWord
PrvFlashAMDProgramAPILoad (FlashAMDGlobalsType* gP)
{
  DWord		err = 0;
  FlashAMDAccessEntryFuncType*	entryP = NULL;
  DmOpenRef	dbP = 0;
  VoidHand	h = 0;
  UInt32		resSize;
  ...
  //-------------------------------------------------------------------------
  entryP = (FlashAMDAccessEntryFuncType*)gP->common.codeCopyP;
  err = entryP (amdAccessEntrySelGetInterface, &gP->program, sizeof(gP->program));
...
/***************************************************************
 *	Function:	PrvFlashAMDProgramAPIUnload
 *
 *	Summary:
...
PrvErrorCheckGlobals (FlashAMDGlobalsType* gP)
{
  ...
  ErrNonFatalDisplayIf (gP == NULL, "null AMD Fld globals");
  ErrNonFatalDisplayIf (gP->startSignature != flashAMDGlobalsStartSignature,
		  "bad Fld globals start signature");
  ErrNonFatalDisplayIf (gP->endSignature != flashAMDGlobalsEndSignature,
  ...
  if (gP->device.identified)
	  ...
	  // DOLATER... revisit this, comment out for now due to 4KB STI
	  //ErrNonFatalDisplayIf (gP->device.maxSectorBytes != (64UL * 1024UL),
	  //						  "bad maxSectorBytes");
