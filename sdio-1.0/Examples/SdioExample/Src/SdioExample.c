 ...
 *Then exit this App.
 *Insert Palm test SDIO card, and this app will auto start.
...
FtrWorkSpaceType ftrWorkSpace;
	...
	*workspacePP = NULL;
	...
	err = FtrPtrNew( appFileCreator, SdioExFeatureWorkSpace, sizeof(*ftrWorkSpaceP), &ftrWorkSpaceP );
	if ( err ) 
	{
		goto Error; 
	}
...
Err errIsr;
	...
	if ( forceUpdate || workspaceP->boardState.ledColorUpdate )
	{
		...
		{
		case LED_RED:
			direct.byteData =  PalmTestSDIOCardFunc1Addr0LedOn|PalmTestSDIOCardFunc1Addr0LedRed;
			break;
			
		case LED_GREEN:
			direct.byteData =  PalmTestSDIOCardFunc1Addr0LedOn|PalmTestSDIOCardFunc1Addr0LedGreen;
			break;
		default: 	//LED_OFF
			direct.byteData =  0;
		...
		direct.byteAddress = PalmTestSDIOCardFunc1Addr0;
		err = SDIORWDirect( workspaceP->slotDriver.slotLibRefNum, &direct );
		if ( err )
		{
			workspaceP->boardState.ledColorUpdate = 1;
