...
// TODO: SysTicksPerSecond not yet defined
#ifndef SysTicksPerSecond
#define SysTicksPerSecond() (1000)
#endif

// session max
#define kSessionCountMax ((uint16_t) 16)

#define TRACE_MODULE TraceDefine(blthErrorClass, 10)
...
typedef struct
{
	uint16_t slaveNumber;			// Tells slave what number they are in the Piconet (1-7)
	uint16_t seqNum;				// Sequential number to verify the packet is pinged correctly
...
void BtLibMECallbackProc(BtLibManagementEventType * mEventP, uint32_t refCon);
void BtLibSocketCallbackProc(BtLibSocketEventType * sEventP, BtLibSocketRef socketRef);
...
// General variables
static uint16_t gSavedAutoOff;	// Disable the auto-power off function while this app is running
static uint16_t gState;			// Bit fields: keeps track of what we need to cleanup

static DmOpenRef gResourceDbRef;	// The local ID of this app's resource data base.

static char gStatus[kMsgLength];	// String to store status information for display
static uint32_t gTimeToClearError;	// Errors are cleared after some elapsed period
static char gBdAddrStr[18];		// Used to covert BD_ADDR to something printable

// Bluetooth variables used by both master and slave
static int32_t gFdME;
...
static struct pollfd gFdSet[kSessionCountMax];	// Set of client fds
static int32_t gIFdCount;		// Number of fds used
...
static uint16_t gNextSlaveToPing;	// Next slave to be ping'ed by the master
static uint32_t gLastPing;		// Time (seconds) when last ping was performed
static uint16_t gSequenceNum;		// Sequence number for ping packets
static BtLibDeviceAddressType gDeviceArray[kMaxSlaves];	// Array of BD_ADDRs to connect to
static uint8_t gNextSlaveToConnectTo;	// Next slave in array to connect to
...
static status_t DiscoverAndConnectMultiple(void);
static status_t AclCreateLink(BtLibDeviceAddressType * deviceArray);
static void GetPSMUsingSDP(void);
static Boolean NoMoreSlaves(void);
static Boolean BusyPinging(void);
static void CleanupSlave(uint16_t slaveNo);
static void ReturnToInitialState(void);
static void UpdateLockUnlockButtonLabel(uint8_t state);
static void FindAndOpenLibrary(void);
static void CreateListenerSocketAndRegisterUUID(void);
static void CloseListenerSocketAndUnregisterUUID(void);

static void InitialisePiconetVariables(void);
static void InitialiseStatusDisplay(void);
...
char *ConvertBtErrToString(status_t err);
		...
		case ctlSelectEvent:
			...
			switch (eventP->data.ctlSelect.controlID)
			{
				case MainHostAndWaitButton:
					...
					if (BtLibPiconetCreate(gFdME, true, true) == btLibErrNoError)
					{
						SetBit(kStatePiconetCreated);
						FrmGotoForm(gResourceDbRef, ActionForm);
					}
					else
					{
				...
				case MainFindAndHostButton:
					...
					{
						SetBit(kStatePiconetCreated);
						error = DiscoverAndConnectMultiple();
				...
				case MainJoinButton:
					...
					error = BtLibDiscoverDevices(gFdME, NULL, NULL, false, (BtLibClassOfDeviceType *) & kDeviceFilter, 1, false, &gMasterAddr, 1, NULL);
					gPendingDiscovery = false;

					if (error == btLibErrNoError)
					{
						// Create an ACL link to the master
						error = AclCreateLink(&gMasterAddr);
						if (error == btLibErrPending)
						{
	...
	switch (eventP->eType)
	{
		...
		case ctlSelectEvent:
			switch (eventP->data.ctlSelect.controlID)
			{
				...
				case ActionAddButton:
					...
					(void)DiscoverAndConnectMultiple();
					handled = true;
					break;
		...
		case SlaveMustReplyEvent:
			...
			if (gRole == kRoleSlave)
			{
				...
				BtLibSocketSend(gDataSocket, (uint8_t *) & gData, sizeof(gData));
			...
			if (gRole == kRoleMaster)
			{
				for (; gNextSlaveToPing <= kMaxSlaves; gNextSlaveToPing++)
				{
					if (gSlave[gNextSlaveToPing].sendStatus == kStatusSlaveIdle)
					{
						...
						gData.slaveNumber = HToNS(gNextSlaveToPing);
						BtLibSocketSend(gSlave[gNextSlaveToPing].socket, (uint8_t *) & gData, sizeof(gData));
						
						// Mark that the data has been sent and we are waiting for a reply
						gSlave[gNextSlaveToPing].sendStatus = kStatusWaitingForSlave;
		...
		case ResponseFromSlaveEvent:
			...
			if (gRole == kRoleMaster)
			{
				...
				if (!BusyPinging())
				{
					...
					++gSequenceNum;
					gData.seqNum = HToNS(gSequenceNum);
...
static status_t SocketClose(BtLibSocketRef socketRef)
{
	uint8_t i;
	status_t error;

	error = BtLibSocketClose(socketRef);
	if (error != btLibErrNoError)
	...
	while (i < gIFdCount)
	{
		if (gFdSet[i].fd == socketRef)
		{
			// Remove one entry
			memmove(&gFdSet[i], &gFdSet[i + 1], (gIFdCount - (i + 1)) * sizeof(struct pollfd));
			gIFdCount--;
		}
		else
		{
...
*/
		...
		case btLibManagementEventACLConnectOutbound:
			...
			else
			{
				...
				error = BtLibLinkGetState(gFdME, &gMasterAddr, btLibLinkPref_LinkRole, &role, sizeof(role));
				if (error || role != btLibSlaveRole)
				{
					ReturnToInitialState();
					BtLibLinkDisconnect(gFdME, &gMasterAddr);
					break;
				}
			}
...
//  PingDataType *pingData;
	...
	switch (sEventP->event)
	{
		case btLibSocketEventSdpGetPsmByUuid:
			...
			SocketClose(socketRef);

			// Find the slave that responded
			for (i = 1; i <= kMaxSlaves; i++)
			{
				if (gSlave[i].socket == socketRef)
					break;
			}
			...
			if ((error = sEventP->status) == btLibErrNoError)
			{
				...
				error = BtLibSocketCreate(&gSlave[i].socket, btLibL2CapProtocol);
				if (error == btLibErrNoError)
				{
					gFdSet[gIFdCount].fd = gSlave[i].socket;
					gFdSet[gIFdCount].events = (short) (POLLIN | POLLHUP);
					...
					error = BtLibSocketConnect(gSlave[i].socket, &connectInfo);
				}
			}
		...
		case btLibSocketEventConnectRequest:
			...
			ErrFatalDisplayIf((gListenerSocket != socketRef), "Connect req socket mismatch");

			StatusMsg("Req from %s", ConvertBdAddrToStr(&sEventP->eventData.requestingDevice));
			BtLibSocketRespondToConnection(socketRef, true);
			break;
		...
		case btLibSocketEventConnectedInbound:
			...
			gFdSet[gIFdCount].fd = gDataSocket;
			gFdSet[gIFdCount].events = (short) (POLLIN | POLLHUP);
			gIFdCount++;
...
static void SocketDataReceived(BtLibSocketRef socketRef, char *data, uint8_t dataLen)
{
	uint16_t i;
	EventType event;
	PingDataType *pingData;
	...
	switch (gRole)
	{
		case kRoleMaster:
			...
			for (i = 1; i <= kMaxSlaves; i++)
			{
				...
				{
					// Check that the sequence number is what is expected
					if (NToHS(pingData->seqNum) != gSequenceNum)
						StatusMsg("Packet out of sequence - slave %i", i);

					// Check that the packet came from the correct slave
					if (NToHS(pingData->slaveNumber) != i)
						ErrorMsg("Packet contains incorrect slave number");
...
//                  StatusMsg("Response from slave %i", i);
		...
		case kRoleSlave:
			// Copy the data into global storage so it isn't lost for the response
			memmove(&gData, data, sizeof(gData));
...
static status_t DiscoverAndConnectMultiple(void)
{
	...
	gPendingDiscovery = true;
	error = BtLibDiscoverDevices(gFdME, NULL, NULL, false, (BtLibClassOfDeviceType *) & kDeviceFilter, 1, false, gDeviceArray, kMaxSlaves, &numDevicesSelected);
	gPendingDiscovery = false;
	...
	if (error == btLibErrNoError)
	{
		...
		gNextSlaveToConnectTo = numDevicesSelected;
 ...
 *
 ***********************************************************************/
static status_t AclCreateLink(BtLibDeviceAddressType * deviceAddress)
{
	status_t error;

	error = BtLibLinkConnect(gFdME, deviceAddress);
	if (error != btLibErrPending)
	{
...
static void GetPSMUsingSDP(void)
{
	...
	for (i = 1; i <= kMaxSlaves; i++)
	{
		...
		{
			// Insist that the socket is not already in use
			ErrFatalDisplayIf((gSlave[i].socket != kInvalidSocket), "Getting PSM - socket in use");

			// Create an SDP socket         
			if (BtLibSocketCreate(&gSlave[i].socket, btLibSdpProtocol) != btLibErrNoError)
			{
			...
			// Use SDP to get the L2CAP PSM that the slave is listening on..  Once the query
			// has completed, the btLibSocketEventSdpGetPsmByUUID callback event will be generated.
			error = BtLibSdpGetPsmByUuid(gSlave[i].socket, &gSlave[i].bdaddr, (BtLibSdpUuidType *) & kAppUUID, 1);
...
static void CleanupSlave(uint16_t slaveNo)
{
	...
	{
		SocketClose(gSlave[slaveNo].socket);
	...
	{
		BtLibLinkDisconnect(gFdME, &gSlave[slaveNo].bdaddr);
...
//      BtLibRegisterManagementNotification(BtLibMECallbackProc, 0);
		StatusMsg("BtLib Opened");
		SetBit(kStateLibraryOpened);
		SetBit(kStateRadioOkay);

		gFdSet[0].fd = gFdME;
		gFdSet[0].events = (short) (POLLIN | POLLHUP);
...
static void CreateListenerSocketAndRegisterUUID(void)
{
	...
	error = BtLibSocketCreate(&gListenerSocket, btLibL2CapProtocol);
	if (error == btLibErrNoError)
	{
		SetBit(kStateListenerSocketCreated);

		gFdSet[gIFdCount].fd = gListenerSocket;
		gFdSet[gIFdCount].events = (short) (POLLIN | POLLHUP);
	...
	if (error == btLibErrNoError)
	{
		...
		error = BtLibSdpServiceRecordSetAttributesForSocket(gListenerSocket, (BtLibSdpUuidType *) & kAppUUID, 1, kAppBtName, strlen(kAppBtName), gSdpRecordH);

		if (error == btLibErrNoError)
			error = BtLibSdpServiceRecordStartAdvertising(gFdME, gSdpRecordH);
...
static Boolean AppHandleEvent(EventPtr eventP)
{
	...
	if (eventP->eType == frmLoadEvent)
	{
		...
		frmP = FrmInitForm(gResourceDbRef, formId);
...
//      EvtGetEvent(&event, timeout);
		...
		if (gIFdCount)
		{
//          error = IOSPoll(gFdSet, gIFdCount, -1, &oFdCount);
			error = IOSPoll(gFdSet, gIFdCount, 1, &oFdCount);
			if (error != errNone)
			{
				TraceOutput(TL(TRACE_MODULE, "BTPiconet: Error returned by poll. Error=%hx", error));
			}
			else
			{
				if (gFdSet[0].revents & POLLIN)
				{
					TraceOutput(TL(TRACE_MODULE, "BTPiconet: Receiving event on ME device"));
					aDataptr.buf = buffer;
					aDataptr.len = 0;
					...
					aIoFlags = 0;
					result = IOSGetmsg(gFdSet[0].fd, &aCtlptr, &aDataptr, &aIoFlags, &error);

					if ((status_t) result == (status_t) - 1)
					{
						TraceOutput(TL(TRACE_MODULE, "BTPiconet: Error returned by read. Error=%hx", result));
					}
					else
					{
						if (aCtlptr.len != -1)
						{
							BtLibMECallbackProc(&MEEvent, 0);
						}
					}
				}

				for (index = 1; oFdCount != 0 && index < gIFdCount; index++)
				{
//                  if (gFdSet[index].revents != 0)
					if (gFdSet[index].revents & POLLIN)
					{
//                      extraMessage.buf = NULL;
						...
						aIoFlags = 0;
						result = IOSGetmsg(gFdSet[index].fd, &aCtlptr, &aDataptr, &aIoFlags, &error);
						...
						else
						{
							if (aCtlptr.len != -1)
							{
								BtLibSocketCallbackProc(&SocketEvent, gFdSet[index].fd);
							}
							else
							{
								SocketDataReceived(gFdSet[index].fd, buffer, aDataptr.len);
							}
						}
					}
				}
...
static void DisplayPiconetStatus(PingDataType * ping)
{
	...
	if (gRole == kRoleSlave)
	{
		sprintf(str, "Slave %i", NToHS(ping->slaveNumber));
	...
	for (i = 1; i <= kMaxSlaves; i++)
	{
		...
		{
			case kAnimNoSlave:
				hRsc = DmGetResource(gResourceDbRef, bitmapRsc, NoLinkBitmapFamily);
				break;

			case kAnimPingError:
				hRsc = DmGetResource(gResourceDbRef, bitmapRsc, UnknownStateBitmapFamily);
				break;

			default:
				// This is one of the animation icons
				iconOffset = 100 * (ping->slaveState[i] - kAnim1);
				hRsc = DmGetResource(gResourceDbRef, bitmapRsc, Action1BitmapFamily + (DmResourceID) iconOffset);
				break;
		}
...
static void UpdateLockUnlockButtonLabel(uint8_t state)
{
	...
	switch (state)
	{
		case kButtonUnknown:
			...
			error = BtLibGetGeneralPreference(gFdME, btLibPref_CurrentAccessible, &access, sizeof(access));
			if (error == btLibErrNoError)
			{
				switch (access)
				{
...
static void StatusMsg(const char *formatStr, ...)
{
	...
	// Display the message
	TraceOutput(TL(TRACE_MODULE, "BTPiconet: %s", gStatus));
	UpdateStatusDisplay();
}
...
static void ErrorMsg(const char *formatStr, ...)
{
	...
	// Display the error
	TraceOutput(TL(TRACE_MODULE, "BTPiconet: ** ERROR ** %s", gStatus));
	UpdateStatusDisplay();
...
static char *ConvertBdAddrToStr(BtLibDeviceAddressType * bdAddrP)
{
	BtLibAddrBtdToA(bdAddrP, gBdAddrStr, sizeof(gBdAddrStr));
	return (gBdAddrStr);
}
