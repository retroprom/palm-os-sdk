...
/** @def cpmErrNoAppContext
 * The CPM application context could not be found for this operation
 */
 ...
 *
 * Structure to hold the various information about the derivation
 * function and parameters. 
 */
struct APDerivedKeyInfoStruct {
  /*! the providerContext of this InfoType */
  APProviderContextType providerContext;
  /*! the type of key derivation to be performed */
  APKeyDerivationEnum kdType;
  /*! the allowed usage of the subsequently derived key */
  APKeyDerivationUsageEnum kdUsage;
  /*! cryptographic salt to be used for the derivation function */
  uint8_t *salt;
  /*! the length of the @c salt */
  uint32_t saltLen;
  /*! the number of iterations for this particular derivation
   * operation 
   */
  uint32_t iterationCount;
  /*! other derivation function specific parameters as defined by both
   * the provider handling the derivation and the derivation function
   * itself
   */
  void *kdInfo;
};

typedef struct APDerivedKeyInfoStruct APDerivedKeyInfoType, *APKeyDerivedKeyInfoPtr;
 ...
 * 
 * Structure to hold information about a particular Message
 * Authentication Code (MAC) operation.
 */
struct APMACInfoStruct {
  /*! the providerContext of this InfoType */
  APProviderContextType providerContext; 
  /*! cipher type - APMACEnum */
  APMACEnum type;       
};

typedef struct APMACInfoStruct APMACInfoType, *APMACInfoPtr;
...
typedef uint32_t VerifyResultType, *VerifyResultPtr;

/** @struct APSignInfoStruct
 * 
 * Structure to hold information about a particular operation's
 * signature context including generic information about signature
 * contexts and specific information about the provider's concept of
 * the signature context.
 *
 * The application is always repsonsible for allocating and freeing
 * the APSignInfoType and the associated APCipherInfoType and
 * APHashInfoType. The application must call
 * CPMLibReleaseCipherInfo on the APCipherInfoType to allow the CPM and
 * provider(s) to cleanup the APCipherInfoType. The application must call 
 * CPMLibReleaseHashInfo on the APHashInfoType to allow the CPM and provider(s)
 ...
 * either the APCipherInfoType or the APHashInfoType, the application
 * would allocate the APCipherInfoType and the APHashInfoType and
 * zero the structures. Upon return from CPMLibSign[Init] the
 * APCipherInfoType structure and APHashInfoType structure would be
 * filled in by the CPM and provider(s) with the appropriate information.
 *
 * If the application does care about the settings for either the
 * APCipherInfoType or the APHashInfoType, the application would
 * allocate the APCipherInfoType and the APHashInfoType and set the
 * fields appropriately before passing it in to CPMLibSign[Init]. The
 * application may set the fields in one or the other or both. 
 */
struct APSignInfoStruct {
  /*! the providerContext of this InfoType */
...
typedef struct APSignInfoStruct APSignInfoType, *APSignInfoPtr;
