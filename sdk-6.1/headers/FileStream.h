...
#define	fileModeLeaveOpen				(0x08000000UL)			// no longer supported (will be ignored)
...
typedef enum FileOpEnum {
	...
	fileOpGetCreatedStatus,				// find out whether the FileOpen call caused the file to
												...
												//		valueLenP = ptr to int32_t variable set to sizeof(Boolean)
												// RETURNS:
												//		zero on success; fileErr... on error;
	...
	fileOpGetOpenDbRef,					// get the open database reference (handle) of the underlying
												...
												//		valueLenP = ptr to int32_t variable set to sizeof(DmOpenRef)
												// RETURNS:
												//		zero on success; fileErr... on error;
...
FileHand FileOpen(const char * nameP, uint32_t type, uint32_t creator, 
	uint32_t openMode, status_t *errP);	

FileHand FileOpenV50(uint16_t cardNo, const char * nameP, uint32_t type, uint32_t creator,
	uint32_t openMode, status_t *errP);
	
// Close the file stream
status_t FileClose(FileHand stream);

// Delete a file
status_t FileDelete(const char *nameP, uint32_t creator);

status_t FileDeleteV50(uint16_t cardNo, const char *nameP);
 ...
 *
 * PROTOTYPE:	int32_t FileRead(FileHand stream, void *bufP, int32_t objSize, int32_t numObj, status_t *errP)
 *
 ...
 *						or resource.
 *
 * PROTOTYPE:	int32_t FileDmRead(FileHand stream, void *startOfDmChunkP, int32_t destOffset,
 *							int32_t objSize, int32_t numObj, status_t *errP)
 *
 * PARAMETERS:	stream		-- handle of open file
 *					startOfDmChunkP
...
// (errP is optional - set to NULL to ignore)
int32_t FileReadLow(FileHand stream, void *baseP, int32_t offset, Boolean dataStoreBased, int32_t objSize,
	int32_t numObj, status_t *errP);

// Write data to a file stream
// (errP is optional - set to NULL to ignore)
int32_t FileWrite(FileHand stream, const void *dataP, int32_t objSize,
	int32_t numObj, status_t *errP);

// Set position within a file stream
status_t FileSeek(FileHand stream, int32_t offset, FileOriginEnum origin);
...
int32_t FileTell(FileHand stream, int32_t *fileSizeP, status_t *errP);

// Truncate a file
status_t FileTruncate(FileHand stream, int32_t newSize);

// Returns the error code from the last operation on this file stream;
// if resetLastError is non-zero, resets the error status
status_t FileControl(FileOpEnum op, FileHand stream, void *valueP, int32_t *valueLenP);
