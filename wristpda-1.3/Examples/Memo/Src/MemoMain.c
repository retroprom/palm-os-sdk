...
#if WRISTPDA
static FontID					ListFont = FossilLargeFontID( WRISTPDA, boldFont );
static FontID					EditFont = FossilLargeFontID( WRISTPDA, boldFont );
#else
...
// Flag to indicate if edit view form is open.
static Boolean EditViewFormOpen = false;
...
static void EditViewUpdateScrollButtons( void );
...
static void ListViewUpdateScrollButtons( UInt16 visibleRows,
                                         UInt16 topVisibleRecord,
...
void MemoLoadPrefs(UInt32*	currentRecordID)
{
	...
	prefsVersion = PrefGetAppPreferences (WPdaCreator, 'Me', &prefs, &prefsSize, true);
	...
	if (needFontInfo)
	{
		...
		#if WRISTPDA
		defaultFont = boldFont; // Converted to Fossil font below.
		#endif
	...
	// Convert for Fossil font
	ListFont = FossilLargeFontID( WRISTPDA, ListFont );
	EditFont = FossilLargeFontID( WRISTPDA, EditFont );
...
void MemoSavePrefs(UInt16 scrollPosition)
{
	...
	prefs.v20editFont = FossilNormalFontID( WRISTPDA, stdFont );
	prefs.v20listFont = FossilNormalFontID( WRISTPDA, stdFont );
	...
	PrefSetAppPreferences (WPdaCreator, 'Me', memoPrefsVersionNum, &prefs,
	...
	if ( prefs.editFont == FossilLarge8Font )
		prefs.editFont = largeFont;

	if ( prefs.editFont == FossilLargeBold8Font )
		prefs.editFont = largeBoldFont;
...
/***********************************************************************
 *
 * FUNCTION:    EditViewUpdateScrollButtons
 *
...
static void EditViewUpdateScrollButtons( void )
{
	...
	// We only update the scroll buttons if the edit view form is open.

	if ( EditViewFormOpen == false )
		return;

	fld = GetObjectPtr (EditMemoField);

	category = ShowAllCategories ? dmAllCategories : CurrentCategory;

	// Update the Down button state.

	ctlDown = GetObjectPtr( EditPageDown );

	// If we can scroll the field then we don't need to scroll the record.

	if ( FldScrollable( fld, winDown ) ) {
		Down = true;
	} else {
		// We can't scroll the field, so check if we can scroll the record.
		recordNum = CurrentRecord;
		while (!DmSeekRecordInCategory (MemoDB, &recordNum, 1, dmSeekForward, category) &&
			   !DmRecordInfo (MemoDB, recordNum, &attr, NULL, NULL) &&
			   ((attr & dmRecAttrSecret) && PrivateRecordVisualStatus == maskPrivateRecords)) ;
		Down = (recordNum > CurrentRecord);
	}
	...
	if ( FldScrollable( fld, winUp ) ) {
		...
		// We can't scroll the field, so check if we can scroll the record.
		recordNum = CurrentRecord;
		while (!DmSeekRecordInCategory (MemoDB, &recordNum, 1, dmSeekBackward, category) &&
			   !DmRecordInfo (MemoDB, recordNum, &attr, NULL, NULL) &&
			   ((attr & dmRecAttrSecret) && PrivateRecordVisualStatus == maskPrivateRecords)) ;
		Up = (recordNum < CurrentRecord);
	}
...
static void EditViewSaveRecord (void)
{
	...
	else
	{
		...
		#if WRISTPDA
		ToolsSeekRecord( & CurrentRecord, 0, -1 );
		#else
		CurrentRecord = noRecordSelected;
		#endif
		MemosInCategory--;
...
static void ListViewDoCommand (UInt16 command)
{
	...
	case ListOptionsSecurityCmd:
		...
		#if WRISTPDA
		{
			...
			RecordsInCategory = DmNumRecordsInCategory( MemoDB, CurrentCategory );
			if ( ( RecordsInCategory > 0 ) && ( CurrentRecord != noRecordSelected ) ) {
				UInt16 attr;
				// Get the category and secret attribute of the current record.
				DmRecordInfo( MemoDB, CurrentRecord, &attr, NULL, NULL );
				// If this is a hidden record, then seek to a visible record.
                                         ...
                                         UInt16 memosInCategory )
{
	...
	// Update the Down button state.
	ctl = GetObjectPtr( ListPageDown );
	enabled = ( ( topVisibleRecord + visibleRows ) < memosInCategory );
	CtlSetLabel( ctl, enabled ? DownArrowEnabled : DownArrowDisabled );
	CtlSetEnabled( ctl, enabled );
	CtlSetUsable( ctl,  ( memosInCategory > visibleRows ) );

	// Update the Up button state.
	ctl = GetObjectPtr( ListPageUp );
	enabled = ( topVisibleRecord > 0 );
	CtlSetLabel( ctl, enabled ? UpArrowEnabled : UpArrowDisabled );
	CtlSetEnabled( ctl, enabled );
	CtlSetUsable( ctl,  ( memosInCategory > visibleRows ) );
...
static void ListViewUpdateScrollers (FormPtr UNUSED_PARAM(frm))
{
	...
	#if WRISTPDA
    ListViewUpdateScrollButtons( rows, TopVisibleRecord, MemosInCategory );
	#else
...
static void RationalizeCurrentSelection( void )
{
	...
	UInt16 RecordsInCategory = DmNumRecordsInCategory( MemoDB, CurrentCategory );
...
static void ListViewPageScroll (WinDirectionType direction)
{
	...
	if ( TblFindRowID( table, SelectedRecord, & SelectedRow ) )
		ListViewSelectTableItem( false, table, SelectedRow, 0, & r );
	if ( DmNumRecordsInCategory( MemoDB, CurrentCategory ) == 0 ) {
	...
	if ( SelectedRecord != noRecordSelected ) {
		if ( TblFindRowID( table, SelectedRecord, & SelectedRow ) ) {
			CurrentRecord = SelectedRecord;
			ListViewSelectTableItem( true, table, SelectedRow, 0, & r );
		} else
			CurrentRecord = SelectedRecord = TopVisibleRecord = noRecordSelected;
	}
	#endif
...
static void ListViewSelectMemo (EventType * event)
{
	...
	#if WRISTPDA
	// Initialize recordNum since we always use it later.
	recordNum = selectedRecord;
	#endif
	...
	while (true)
	{
		...
		else if (! RctPtInRectangle (x, y, &r))
		{
			...
			{
				#if WRISTPDA
				// Indicate that we have moved up.
				MoveUp = true;
				#endif
				recordNum = TblGetRowID (table, row);
			...
			{
				#if WRISTPDA
				// Indicate that we have moved down.
				MoveDown = true;
				#endif
				recordNum = TblGetRowID (table, row);
...
static UInt16 LastVisibleRecord( void )
{
	...
	NumRows = ListViewNumberOfRows( GetObjectPtr( ListTable ) );
	// Why is NumRows - 1 needed here instead of NumRows?
	...
	if (event->eType == keyDownEvent)
	{
		...
		else if (EvtKeydownIsVirtual(event))
		{
			...
			else if (event->data.keyDown.chr == vchrPageDown)
			{
				...
				// Open the selected record in Edit View.
				UInt16 RecordsInCategory;
				RecordsInCategory = DmNumRecordsInCategory( MemoDB, CurrentCategory );
				if ( ( RecordsInCategory > 0 ) && ( CurrentRecord != noRecordSelected ) ) {
					UInt16 attr;
