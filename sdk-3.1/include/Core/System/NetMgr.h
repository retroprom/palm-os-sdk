...
typedef struct NetSocketAddrRawType {
	SWord			family;					// Address family in HBO (Host Byte Order)
	Word			ifInstance;				// the interface instance number 
	DWord			ifCreator;				// the interface creator
...
typedef enum {
	...
	//---------------------------------------------------------------------------------
	netIFSettingPowerUp,						// Byte, true if this interface is powered up
	...
	netIFSettingWireless,					// Byte, true if this interface is wireless
	


	// Option to query server for address of DNS servers
	netIFSettingDNSQuery,					// Byte, if true PPP queries for DNS address. Default true
	...
	netIFSettingRawMode,						// DWord, parameter to pass to raw_rcv() along with
													//  packet pointer. 
...
// used over wireless. 
//--------------------------------------------------
void				NetLibBitMove(Word libRefNum, BytePtr dstP, DWordPtr dstBitOffsetP,
								BytePtr srcP, DWordPtr srcBitOffsetP,
								DWord		numBits)
						SYS_TRAP(netLibTrapBitMove);

								
void				NetLibBitPutFixed(Word libRefNum, BytePtr dstP, DWordPtr dstBitOffsetP,
								DWord value, Word numBits)
						SYS_TRAP(netLibTrapBitPutFixed);
								
DWord 			NetLibBitGetFixed(Word libRefNum, BytePtr dstP, DWordPtr dstBitOffsetP, 
								Word numBits)
						SYS_TRAP(netLibTrapBitGetFixed);
								
void			  	NetLibBitPutUIntV(Word libRefNum, BytePtr dstP, DWordPtr dstBitOffsetP, 
								DWord value)
						SYS_TRAP(netLibTrapBitPutUIntV);

DWord 			NetLibBitGetUIntV(Word libRefNum, BytePtr dstP, DWordPtr dstBitOffsetP)
						SYS_TRAP(netLibTrapBitGetUIntV);

void			  	NetLibBitPutIntV(Word libRefNum, BytePtr dstP, DWordPtr dstBitOffsetP, 
								SDWord value)
						SYS_TRAP(netLibTrapBitPutUIntV);

SDWord 			NetLibBitGetIntV(Word libRefNum, BytePtr dstP, DWordPtr dstBitOffsetP)
						SYS_TRAP(netLibTrapBitGetUIntV);
